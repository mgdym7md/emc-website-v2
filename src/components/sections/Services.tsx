  ...
  return (
    <section id="services" className="section-padding bg-dark-secondary relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-30">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-accent-rose/5 rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              className="group p-8 bg-dark-primary/50 border border-accent-gold/10 hover:border-accent-gold/30 transition-all duration-500"
            >
              {/* Icon */}
              <div className="w-16 h-16 flex items-center justify-center text-accent-gold/70 group-hover:text-accent-gold mb-6 transition-colors">
                {iconMap[service.icon] || iconMap.globe}
              </div>

              {/* Content */}
              <h3 className="font-heading text-xl text-accent-cream group-hover:text-accent-gold mb-4 transition-colors">
                {service.title}
              </h3>
              <p className="text-accent-cream/60 leading-relaxed">
                {service.description}
              </p>

              {/* Hover Line */}
              <div className="mt-6 h-px bg-gradient-to-r from-accent-gold/0 via-accent-gold/50 to-accent-gold/0 scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
            </motion.div>
          ))}
        </div>
      </div>
